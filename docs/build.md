...
This 1.132 line uses **password / app-password** sign-in. AT Protocol OAuth
client metadata and env vars are in [docs/oauth.md](./oauth.md); completing
OAuth login requires a SessionBundle adapter. Launchable web OAuth lives on
the 1.109 Community Notes line (PR #7). Local PDS (`alice.test`) uses the
hosting-provider override plus password login.
