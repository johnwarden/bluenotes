...
- **Notes Service**: Endpoints used by the front-end app for creating, rating, and reading proposals. Internal /score endpoint called by scoring service
- **Labeler**: Provides AT Protocol labeler endpoints. Internal /label endpoint called by notes service
- **Scoring Service**: Runs algorithms to score proposals and calls Notes Service to create labels
...
- **Feed Generator**: Provides `getFeedSkeleton` endpoint
...
- **Ratings**: `social.pmsky.vote` records
...
The `/score` endpoint attempts to sync `pendingLabels` to labeler after each call.
...
When users use a community-notes-enabled client:
...
1. **Labeler starts first** → Ready to create labels
2. **Notes Service starts next** → Creates/migrates notes service DB
...
- **All services run locally** with test databases
