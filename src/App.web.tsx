    ...
    async function onLaunch(account?: SessionAccount) {
      try {
        ...
        if (oauthResult?.session && 'state' in oauthResult) {
          // Consume the redirect so the fragment is not replayed, but do not
          // attempt PasswordSession.login — DPoP tokens are not JWTs.
          logger.warn(
            'OAuth callback received but 1.132 PasswordSession cannot complete sign-in',
          )
          Toast.show(
            l`OAuth sign-in is not available on this build. Please use your password or app password.`,
            {type: 'info'},
          )
          clearOauthCallbackUrl()
        }
